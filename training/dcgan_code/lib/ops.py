...
from theano.gpuarray.basic_ops import (host_from_gpu,
...
def conv_cond_concat(x, y): #XXX
    """ 
    concatenate conditioning vector on feature map axis 
    """
    # '*'演算子は、要素ごとの積。左右は同じサイズでなければならない。
    tmp = y*T.ones((x.shape[0], y.shape[1], x.shape[2], x.shape[3]))
    return T.concatenate([x, tmp], axis=1)

def conv_cond_concat2(x, y): #XXX for test
    """ 
    concatenate conditioning vector on feature map axis 
    """
    # '*'演算子は、要素ごとの積。左右は同じサイズでなければならない。
    tmp = y*T.ones((x.shape[0], y.shape[1], x.shape[2], x.shape[3]))
    return T.concatenate([x, tmp], axis=1), tmp
